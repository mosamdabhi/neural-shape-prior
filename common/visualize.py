...
def get_figure3d(
    joint_connections, input_cam_x, input_cam_y, points3d, points3d_gt, range_scale=2500
):
...
def process_dataset(args, GT_Flag, pkl_names):
    ...
    for cam_idx in range(len(pkl_names)):
        ...
        for frame_idx in range(len(pickled_data)):
            ...
            S_field_name = "S_" + args.field_of_validation
    ...
    return W_Pred, S_Pred, W_GT, S_GT
...
def main(args):
    ...
    W_Pred, S_Pred, W_GT, S_GT = process_dataset(args, GT_Flag, pkl_names)
    ...
    S_GT = S_GT - S_GT.mean(1, keepdims=True)

    S_Pred = S_Pred - S_Pred.mean(1, keepdims=True)

    if args.plot_improvement_over_iterations:
        S_Pred_Full_ = np.load(dataset + "/Predictions.npz")["S_Pred_Full"]
        W_Pred_Full = np.load(dataset + "/Predictions.npz")["W_Pred_Full"]
        S_Pred_Full = []
        for idx in range(S_Pred_Full_.shape[0]):
            S_tmp = S_Pred_Full_[idx] - S_Pred_Full_[idx].mean(1, keepdims=True)
            S_Pred_Full.append(S_tmp)
        S_Pred_Full = np.asarray(S_Pred_Full)

    PA_MPJPE_3D = computeMPJPE(torch.from_numpy(S_Pred), torch.from_numpy(S_GT)) * 1000
    ...
    if args.mode_of_validation == "2D":
        ...
        for cam_idx in range(total_views):
            image_paths_cam_ = []
            for path in Path(dataset_image + "/CAM_" + str(cam_idx + 1) + "/").rglob(
                "*" + str(args.img_type)
            ):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--field_of_validation", help="Decide b/w Inliers, Flow, Detector, MV", default="Inliers",
    )
    ...
    parser.add_argument(
        "--plot_improvement_over_iterations",
        type=parse_boolean,
        default=False,
        help="Flag deciding plottting for improvement over iterations.",
    )
    parser.add_argument(
