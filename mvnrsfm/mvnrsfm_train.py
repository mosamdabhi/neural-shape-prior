...
def main(args):
    ...
    for itr_num in range(mv_nrsfm_kernel.max_itr):
        ...
        for idx in range(len(W)):
            if ~(torch.all(torch.any(torch.any(torch.isnan(W[idx][index_, ...]), dim=1), dim=1,))):
        ...
        Err, Packet_2D, Packet_3D = mv_nrsfm_kernel.validation(data_loaded)
        ...
        if (
            ...
            Errors, Packet_2D, Packet_3D = mv_nrsfm_kernel.validation(data_loaded)
            W_Pred_Full.append(Packet_2D.detach().cpu().numpy())
            S_Pred_Full.append(Packet_3D[0, :, :, :].detach().cpu().numpy())
            ...
            if current_error < best_error:
                ...
                field_names = ["W_MV_Train", "S_MV_Train"]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--unittest", type=parse_boolean, default=False, help="Flag for unit test.",
    )
